...
function moduleReport (report) {
  ...
  const { name, version, failures, license } = report[0]
  ...
  if (license.pass === true) {
    ...
    reportFailMsg(`Noncompliant: ${license.title.split(':')[1].trim().replace(/['"]+/g, '') || ''}`)
  }
  L()
...
function vulnerabilityList (report) {
  let netSecurity = {
    'LOW': 0,
    'MEDIUM': 0,
    'HIGH': 0,
