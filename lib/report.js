...
function moduleReport (report) {
  ...
  if (securityVulnerabilities.length > 0) {
    for (const { severity, title, data } of securityVulnerabilities) {
      ...
      L(chalk`{rgb(137,161,157) Versions ${data.vulnerable.join(' ')} (Source: Synk)}`)
  ...
  if (moduleRisks.length > 0) {
    moduleRisks.forEach(({ title }, ind) => {
      ...
      if (title.length > maxLength) {
        ...
        chalk`{white  ${lines[0]}} ${' '.repeat(maxLength - lines[0].length)}{rgb(137,161,157) │}`)
        L(chalk`{rgb(137,161,157) │} {rgb(255,96,64)  } {rgb(137,161,157) │}` +
        chalk`{white  ${lines[1]}} ${' '.repeat(maxLength - lines[1].length)}{rgb(137,161,157) │}`)
      } else {
        L(chalk`{rgb(137,161,157) │} {rgb(255,96,64) X} {rgb(137,161,157) │}` +
