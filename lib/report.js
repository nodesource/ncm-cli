...
function shortReport (report) {
  ...
  L()
  if (pkgjson) {
    logHeader(`${pkgjson.name} Report`)
  } else {
    // Use a general `NCM-Cli Report` instead of cwd?
    logHeader(`${process.cwd()} Report`)
  }
  ...
  /* Compliance */
  L(chalk`  {rgb(255,96,64) !} {white ${netCompliance} noncompliant modules found}`)
  L()
}
...
function moduleList (report) {
  ...
  report.forEach((pkg, ind) => {
    ...
    chalk`${' '.repeat(W[2] - license.length - 2)}{rgb(137,161,157) │} ` +
    securityBadges.join('') +
