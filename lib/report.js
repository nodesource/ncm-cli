...
function shortReport (report, dir) {
  ...
  /* Compliance */
  const complianceBadge = netCompliance > 0 ? `${COLORS.red} !` : `rgb(90,200,120) ✓`
...
function moduleList (report) {
  ...
  report.forEach((pkg, ind) => {
    ...
    const licenseBadge = pkg.license.pass ? chalk`{rgb(90,200,120) ✓}` : chalk`{${COLORS.red} X}`
    ...
    const securityBadges = [
      vulns.reduce((a, b) => a + b, 0) === 0 ? chalk`{rgb(90,200,120) ✓} ` + chalk`{white 0}` : chalk`{${COLORS.red} X} `,
...
function shortVulnerabilityList (report) {
  ...
  const securityBadge = vulnerabilityCount > 0 ? `${COLORS.red} !` : `rgb(90,200,120) ✓`
...
function severityMeter (severity) {
  switch (severity.toUpperCase()) {
    case 'CRITICAL':
      return [chalk`{${COLORS.red} ||||}`, chalk`{rgb(76,88,89) } `]
    case 'HIGH':
      return [chalk`{${COLORS.orange} |||}`, chalk`{rgb(76,88,89) |} `]
    case 'MEDIUM':
      return [chalk`{${COLORS.yellow} ||}`, chalk`{rgb(76,88,89) ||} `]
    case 'LOW':
      return [chalk`{${COLORS.light1} |}`, chalk`{rgb(76,88,89) |||} `]
    case 'NONE':
      return [chalk`{${COLORS.light1} }`, chalk`{rgb(76,88,89) ||||} `]
  }
}
