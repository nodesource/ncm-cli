...
const analyze = async ({
  ...
  for (const batch of batches) {
    ...
    await Promise.all([...batch].map(async page => {
      const fetchedData = await fetchData({ pkgs: page, token, url, dir });
...
const fetchData = async ({ pkgs, token, url, dir }) => {
  // First attempt to use local certification via ncm-ng
  try {
    // Try to use local ncm-ng if available
    ...
    // Local certification failed, try the API instead
    console.error(`Local certification failed: ${localErr.message}. Falling back to API.`)
  }
  
  // Fall back to using the remote API
  try {
    const query = `
    ...
    return { data: apiData, usingLocalCertification: false }
  } catch (apiErr) {
    throw new Error(`Failed to fetch package data: ${apiErr.message}`)
  }
}
}
