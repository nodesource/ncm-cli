...
function moduleReport (report) {
  ...
  if (license.pass === true) {
    const msg = license.title.split(':')[1].trim().replace(/['"]+/g, '') || ''
    L(boxbox('✓', msg, COLORS.green))
  } else {
    let msg = 'NO LICENSE'
    if (license && license.title && license.title.includes(':')) {
      msg = `Noncompliant: ${license.title.split(':')[1].trim().replace(/['"]+/g, '')}`
    }
