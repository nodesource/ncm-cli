...
function moduleList (report, title, options) {
  ...
  report.forEach((pkg, ind) => {
    ...
    if (pkg.published) {
      ...
      const { score = 0 } = pkg
      const getScoreColor = score => score === 100
        ? COLORS.green
        : score === 0
          ? COLORS.red
          : COLORS.yellow
      ...
      L(
        ...
        chalk`{${COLORS.light1} │} ${mod} {${getScoreColor(score)} (${score})} ${' '.repeat(Math.max(W[0] - (mod.length + String(score).length + 3), 0))}{${COLORS.light1} │}` +
