...
function moduleList (report, title, options) {
  ...
  report.forEach((pkg, ind) => {
    ...
    if (pkg.published) {
      // Extract license from the package data
      ...
      // Determine if the license is compliant (special handling for common licenses)
      let isCompliant = pkg.license && pkg.license.pass

      // Always mark standard open source licenses as compliant (green checkmark)
      const compliantLicenses = ['MIT', 'Apache', 'BSD', 'ISC', 'CC0', 'Unlicense', 'MPL']

      if (!isCompliant && license) {
        const licenseUpper = license.toUpperCase()
        isCompliant = compliantLicenses.some(cl => licenseUpper.includes(cl.toUpperCase()))
      }

      const licenseBadge = isCompliant
        ? chalk`{${COLORS.green} ✓}`
        : chalk`{${COLORS.red} X}`

      /* security badge */
      const vulns = [0, 0, 0, 0]
      // Ensure pkg.failures exists and is an array before iterating
      if (pkg.failures && Array.isArray(pkg.failures)) {
