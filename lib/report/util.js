...
function moduleList (report, title, options) {
  ...
  report.forEach((pkg, ind) => {
    const version = pkg.version != null ? String(pkg.version) : 'N/A'
    /* Module Name */
    ...
    if (pkg.published) {
      const rawSpdx = pkg.license && pkg.license.data && pkg.license.data.spdx
      const normalized = rawSpdx ? licenses.normalize(rawSpdx) : null
      let license = normalized || 'UNKNOWN'
