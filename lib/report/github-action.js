...
const fs = require('fs')
// Update import for @actions/github to fix TypeScript error
let github
try {
  github = require('@actions/github')
} catch (err) {
  // Provide fallbacks if the module is not available
  github = {
...
async function GitHubActionCheck (pkgScores, whitelist, hasFailures) {
  ...
  // Use getOctokit instead of GitHub constructor
  const octokit = github.getOctokit ? github.getOctokit(gToken) : {}
  const contextData = {
  ...
  // annotations.
  const gitOwner = github.context?.repo?.owner || ''
  const gitRepo = github.context?.repo?.repo || ''
  const gitRef = github.context?.sha || ''
      ...
      if (parsedLine !== 1) {
        ...
        if (pkg.failures.length !== 0) {
          for (const failure of pkg.failures) {
            ...
            failureMessages += failure.data == null
              ? ''
              : failure.data.spdx == null
                ? ''
  ...
  // Fix comparison to check array length instead
  if (warningAnnotations && warningAnnotations.length > 0) {
