...
function divider (length, char = '-') {
  return chalk`{${COLORS.light1} ${char.repeat(length)}}`
}
...
function formatError (message, err) {
  ...
  if (err) {
    if (err.code) {
      errMsg = util.format(message, err.code, err)
    } else {
      errMsg = util.format(message, err)
    }
  }

  if (errMsg) {
    if (process.env['NCM_DEV'] === 'true') message = errMsg
    debug(errMsg)
  }
